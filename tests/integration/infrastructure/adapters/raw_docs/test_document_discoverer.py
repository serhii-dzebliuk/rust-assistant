...
def test_discover_skips_rustdoc_redirect_files(workspace_tmp_path: Path):
...
""",
    ...
    discoverer = RawDocsDocumentDiscoverer(raw_dir)
...
def test_discover_keeps_regular_rustdoc_html_files(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
...
def test_discover_skips_std_all_items_index(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
    std_dir = raw_dir / "std"
    std_dir.mkdir(parents=True)
...
def test_discover_skips_cargo_changelog(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
...
def test_discover_skips_book_redirect_files(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
...
def test_discover_skips_book_legacy_alias_pages(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
    book_dir = raw_dir / "book"
    book_dir.mkdir(parents=True)
...
def test_discover_keeps_regular_book_html_files(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
    book_dir = raw_dir / "book"
    book_dir.mkdir(parents=True)
...
def test_discover_skips_book_service_files_but_keeps_index(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
    book_dir = raw_dir / "book"
    book_dir.mkdir(parents=True)
...
def test_discover_skips_second_edition_book_files(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
...
def test_discover_skips_reference_redirect_files(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
...
def test_discover_skips_html_pages_without_meaningful_main_content(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
    ref_dir = raw_dir / "reference"
    ref_dir.mkdir(parents=True)
...
def test_discover_keeps_placeholder_pages_with_real_text(workspace_tmp_path: Path):
    raw_dir = workspace_tmp_path / "raw"
