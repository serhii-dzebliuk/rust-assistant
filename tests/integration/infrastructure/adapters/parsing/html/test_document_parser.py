...
from rust_assistant.application.dto.document_parse import DocumentParseFailureReason
from rust_assistant.domain.value_objects.structured_blocks import BlockType

pytestmark = pytest.mark.integration


def parse_document(page_parser, file_path: Path):
    result = page_parser.parse_file(file_path)
...
def test_reference_destructors_keeps_code_comments_and_removes_rule_anchors(
    ...
    doc = parse_document(page_parser, file_path)
...
def test_book_parse_preserves_structured_blocks_for_headings_and_code(
    ...
    doc = parse_document(page_parser, raw_data_dir / "book/ch01-03-hello-cargo.html")
...
def test_rustdoc_detects_trait_module_and_keyword_item_types(page_parser, raw_data_dir: Path):
    ...
    trait_doc = parse_document(page_parser, raw_data_dir / "std/alloc/trait.Allocator.html")
    assert trait_doc.item_type == "trait"

    module_doc = parse_document(page_parser, raw_data_dir / "std/alloc/index.html")
    assert module_doc.item_type == "module"

    keyword_doc = parse_document(page_parser, raw_data_dir / "std/keyword.async.html")
    assert keyword_doc.item_type == "keyword"

    primitive_doc = parse_document(page_parser, raw_data_dir / "std/primitive.unit.html")
...
def test_rustdoc_uses_rust_fences_for_rustdoc_code_blocks(page_parser, raw_data_dir: Path):
    doc = parse_document(page_parser, raw_data_dir / "std/alloc/struct.Layout.html")
...
def test_rustdoc_parse_preserves_section_paths_and_anchors(page_parser, raw_data_dir: Path):
    doc = parse_document(page_parser, raw_data_dir / "std/vec/struct.Vec.html")
...
def test_rustdoc_code_blocks_do_not_insert_spurious_line_breaks(page_parser, raw_data_dir: Path):
    doc = parse_document(page_parser, raw_data_dir / "std/alloc/trait.Allocator.html")
...
def test_rustdoc_module_index_keeps_item_table_entries(page_parser, raw_data_dir: Path):
    doc = parse_document(page_parser, raw_data_dir / "std/intrinsics/fallback/index.html")
...
def test_reference_parse_preserves_structured_blocks(page_parser, raw_data_dir: Path):
    doc = parse_document(page_parser, raw_data_dir / "reference/types/tuple.html")
    assert doc.structured_blocks[0].block_type == BlockType.HEADING
...
def test_html_parser_returns_structured_failure_for_empty_text(
    page_parser,
    raw_data_dir: Path,
    monkeypatch,
):
    class EmptyTextLayout:
        def extract_title(self, soup, file_path):
            return "Empty"

        def select_main(self, soup):
            return object()

        def clean_main(self, main):
            return main

        def extract_blocks(self, main):
            return []
    ...
    monkeypatch.setattr(
        "rust_assistant.infrastructure.adapters.parsing.html.document_parser.get_layout",
        lambda _source_type: EmptyTextLayout(),
    )
    ...
    assert result.failure.reason == DocumentParseFailureReason.EMPTY_TEXT
