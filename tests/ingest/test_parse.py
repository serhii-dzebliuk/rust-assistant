...
def test_reference_destructors_keeps_code_comments_and_removes_rule_anchors():
    ...
    file_path = Path("data/raw/reference/destructors.html")
...
def test_reference_destructors_keeps_sublist_content():
    parser = PageParser(raw_data_dir=Path("data/raw"))
    file_path = Path("data/raw/reference/destructors.html")
...
def test_book_hello_world_preserves_inline_code_and_avoids_nested_pre_duplicates():
    parser = PageParser(raw_data_dir=Path("data/raw"))
    file_path = Path("data/raw/book/ch01-02-hello-world.html")
...
def test_book_hello_cargo_uses_console_fences_and_preserves_inline_commands():
    parser = PageParser(raw_data_dir=Path("data/raw"))
    file_path = Path("data/raw/book/ch01-03-hello-cargo.html")
...
def test_rustdoc_metadata_uses_canonical_item_path_and_version():
    parser = PageParser(raw_data_dir=Path("data/raw"))
    file_path = Path("data/raw/std/alloc/struct.Layout.html")
    ...
    assert doc.metadata.url == "https://doc.rust-lang.org/std/alloc/struct.Layout.html"


def test_rustdoc_detects_trait_module_and_keyword_item_types():
    parser = PageParser(raw_data_dir=Path("data/raw"))

    trait_doc = parser.parse_file(Path("data/raw/std/alloc/trait.Allocator.html"))
    assert trait_doc is not None
    assert trait_doc.metadata.item_type == "trait"

    module_doc = parser.parse_file(Path("data/raw/std/alloc/index.html"))
    assert module_doc is not None
    assert module_doc.metadata.item_type == "module"

    keyword_doc = parser.parse_file(Path("data/raw/std/keyword.async.html"))
    assert keyword_doc is not None
    assert keyword_doc.metadata.item_type == "unknown"


def test_rustdoc_uses_rust_fences_for_rustdoc_code_blocks():
    parser = PageParser(raw_data_dir=Path("data/raw"))
    doc = parser.parse_file(Path("data/raw/std/alloc/struct.Layout.html"))
...
def test_rustdoc_module_index_keeps_item_table_entries():
    parser = PageParser(raw_data_dir=Path("data/raw"))
    doc = parser.parse_file(Path("data/raw/std/intrinsics/fallback/index.html"))

    assert doc is not None
    assert "std::intrinsics::fallback" == doc.metadata.item_path
    assert doc.metadata.item_type == "module"
    assert "Carrying MulAdd" in doc.text
    assert "Disjoint BitOr" in doc.text


def test_cargo_metadata_uses_page_path_sets_url_and_null_item_type():
    parser = PageParser(raw_data_dir=Path("data/raw"))
    file_path = Path("data/raw/cargo/commands/cargo-build.html")
